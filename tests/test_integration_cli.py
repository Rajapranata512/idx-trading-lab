...
def test_run_daily_end_to_end(tmp_path, monkeypatch):
    ...
    assert "model_v2_shadow" in out
    assert "model_v2_promotion" in out
    assert "model_v2_rollout" in out
    assert "weekly_kpi" in out
    assert "signal_snapshot" in out
    assert "live_reconciliation" in out
    assert Path("reports/model_v2_shadow_signals.json").exists()
    assert Path("reports/model_v2_promotion_state.json").exists()
    assert Path("reports/model_v2_ab_test.json").exists()
    assert Path("reports/weekly_kpi.json").exists()
    assert Path("reports/weekly_kpi.md").exists()
    assert Path(out["beginner_note_path"]).exists()
    assert Path("reports/snapshots").exists()
    assert Path("reports/live_reconciliation.json").exists()
    assert Path("reports/live_reconciliation.md").exists()
...
def test_liquidity_cost_estimate_handles_missing_avg_volume(tmp_path, monkeypatch):
    ...
    plan = pd.DataFrame(
        [
            {"ticker": "BBCA", "mode": "swing", "score": 80.0, "entry": 10000, "stop": 9800, "tp1": 10200, "tp2": 10400, "size": 100, "reason": "ok"},
            {"ticker": "TLKM", "mode": "swing", "score": 75.0, "entry": 4000, "stop": 3900, "tp1": 4100, "tp2": 4200, "size": 100, "reason": "ok"},
        ]
    )
    out = _apply_liquidity_cost_estimate(plan, settings)
    assert len(out) == len(plan)
    assert {"liq_bucket", "est_slippage_pct", "est_roundtrip_cost_pct"} <= set(out.columns)
    assert set(out["liq_bucket"].astype(str).tolist()) == {"low"}
