...
def test_signal_accuracy_audit_writes_reports_and_flags_false_positives(tmp_path: Path) -> None:
    ...
    assert payload["overall"]["trade_count"] > 0
    assert payload["live_threshold"]["trade_count"] > 0
    assert payload["precision_at_k"]["combined"][0]["sample_count"] > 0
    assert payload["calibration"]["status"] == "ok"
    assert payload["threshold_optimization"]["t1"]["candidates"]
    assert payload["false_positive_summary"]["negative_signal_count"] > 0
