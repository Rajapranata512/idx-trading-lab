...
`DATA-01B Official point-in-time universe history expansion` is the single next action.

Acceptance criteria:

1. Inventory the historical date range used by T1/Swing training and report every period
   not covered by an official IDX30/LQ45 constituent snapshot.
2. Add only source-verifiable official periods with non-overlapping `valid_from` and
   `valid_until`; store source URL, announcement identity, and verification date.
3. Preserve point-in-time membership so no row can use a constituent list published
   after that row's market date.
4. Keep production fail-closed outside verified coverage; do not silently substitute
   the current universe for historical dates.
5. Add boundary, overlap, gap, and look-ahead regression tests plus a machine-readable
   coverage report.
6. Do not retrain, recalibrate, or promote Model V2 from expanded history until the
   dataset lineage and label audit pass independently.
7. Record measured coverage and test results here; push/deploy remains explicit.

Likely files: `data/reference/universe_history.csv`, universe update/validation modules,
a small coverage audit report, nearest universe tests, and this PRD.
